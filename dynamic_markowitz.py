...
OUTPUT_DIR = dynamic_config["output_dir"]
...
def plot_garch_evaluation_table(series, max_p=2, max_q=2, filename="garch_evaluation.png", output_dir=dynamic_config["output_dir"]):
...
def fetch_data(asset_tickers, start_date, end_date, csv_path=dynamic_config["csv_path"]):
...
def run_portfolio_analysis(asset_tickers, start_date, end_date, window_size, rebalance_frequency, 
    ...
    if isinstance(asset_tickers, dict):
        ...
        all_tickers = asset_tickers
    data = fetch_data(all_tickers, start_date, end_date, csv_path=dynamic_config["csv_path"])
...
def compute_and_plot_error_metrics(forecasted_returns, actual_returns, dates, output_dir=dynamic_config["output_dir"]):
...
def walk_forward_backtest(asset_tickers, start_date, end_date, window_size, rebalance_frequency,
    ...
    data = fetch_data(asset_tickers, start_date, end_date, csv_path=dynamic_config["csv_path"])
    returns = np.log(data / data.shift(1)).dropna()
    dates = returns.index
...
if __name__ == "__main__":
    ...
    csv_path = dynamic_config["csv_path"]
